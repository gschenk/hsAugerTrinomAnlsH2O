...
multProd bA q is ps
    ...
    | length is /= length ps = error "Unfortunately, array lenghts of indices and probs in function 'sumTerm' do not match."
    | otherwise                  =0
    where   a = last is
            n = head is
            m = (sum . tail) nms
...
prmSeeds q   | q==0  = [zs]
            ...
            | q==5  = map cstr [[4,3,1],[3,1,2]]
            | q==6  = map cstr [[4,2,2],[3,0,3]]
            | q==7  = map cstr [[4,1,3]]
            | q==8  = map cstr [[4,0,4]]
            where zs = [0,0,0,0] -- empty list
                  os = [1,1,1,1]
                  ts = [2,2,2,2]
...
prmInds  = concat . map (nub . permutations) . prmSeeds

-- add auger probs
indsA q a 
    | q >= a    =(map (++[a]) . prmInds) (q-a)
    eeds
